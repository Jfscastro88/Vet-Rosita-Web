...
export default function AnimalAddForm({ onAdd }: { onAdd: (v: AnimalFormValues) => void; }) {
    ...
    return (
        <form onSubmit={form.onSubmit((vals) => { onAdd(vals); form.reset(); })} noValidate>
        <Stack gap="md">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <TextInput label="Nome" required withAsterisk {...form.getInputProps("nome")} />
        <NumberInput label="Età (anni)" required withAsterisk min={0} {...form.getInputProps("eta")} />
        <Select
        label="Sesso"
        required withAsterisk
        data={[{ value: "m", label: "Maschio" }, { value: "f", label: "Femmina" }, { value: "n", label: "Non specificato" }]}
        ...
        {form.values.tipo === "altro" && (
            <TextInput label="Specifica (Altro)" placeholder="es. coniglio, furetto..." {...form.getInputProps("altro")} />
        )}
        </div>
        <Group justify="end">
        <Button type="submit" leftSection={<IconPlus size={18} />}>Aggiungi</Button>
        </Group>
        </Stack>
        </form>
    );
